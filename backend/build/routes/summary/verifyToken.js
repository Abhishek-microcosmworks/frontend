...
export const verifyToken = async (req, res, next) => {
  try {
    ...
    try {
      ...
      return res.status(500).json({ expired: true, message: 'Token verification error' });
    }
    ...
    if (decoded.exp !== undefined && decoded.exp < currentTime) {
      ...
      return res.status(401).json({ expired: true, decoded, message: 'token expired' });
    } else {
      req.decoded = decoded;
      next();

      return res.status(200).json({ expired: false, decoded, newToken: userToken });
    }
  } catch (error) {
