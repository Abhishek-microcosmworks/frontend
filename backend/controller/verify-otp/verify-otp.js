...
export const verifyOtp = async (req, res) => {
  try {
    ...
    if (
      currentTime > otpResponse.data.otpExp &&
      otpResponse.data.isDeleted === false
    ) {
    ...
    if (otp === otpResponse.data.otp) {
      ...
      const authToken = await saveToken(
        token,
        otpResponse.data.email,
        otpResponse.data._id
      );
      ...
      return res.status(200).json({ error: authToken.error, data: authToken.data });
    }
  } catch (error) {
    console.log(error);
